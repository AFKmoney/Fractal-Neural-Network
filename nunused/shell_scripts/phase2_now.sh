#!/bin/bash
cd /root/FNN
# Quick Phase 2 with what we have
cat > phase2_quick.py << 'PYEOF'
...
STEPS = 50000
SAVE_MODEL = "/workspace/checkpoints/NFNmini_phase2.pt"
...
for step in range(1, STEPS + 1):
    ...
    if step % 100 == 0:
        ...
        print(f"[{step:6d}] loss={cl:.3f} best={best:.4f} lr={lr:.2e} "
              f"A={av[0]:.2f} S={av[1]:.2f} P={av[2]:.2f} D={av[3]:.2f} C={av[4]:.2f} T={av[5]:.2f} "
              f"| {e:.0f}s", flush=True)
...
print(f"\nDONE Phase 2 — Best={best:.4f}", flush=True)
PYEOF
