...
from nfn.prism.config import MemoryConfig, PrismConfig
from nfn.prism.holo_data import get_holo_mix, holo_mix_summary, format_retrieval_example
from nfn.prism.holo_loss import (
    holo_consistency_loss,
    holo_contrastive_loss,
    pooled_embedding,
)
from nfn.prism.model import Prism
from nfn.prism.pcs import DEFAULT_SCHEDULE, grow_model, resolve_schedule, schedule_summary
from nfn.prism.train_scale import PRESETS
...
def train(args):
    ...
    if args.smoke:
        ...
        from nfn.prism.data_scale import _load_tokenizer, build_dataloader, batched
        mix = get_holo_mix()
...
def _run_one_stage(model, args, tokenizer, vocab_size, device, is_main):
    ...
    if args.smoke:
        ...
        from nfn.prism.data_scale import build_dataloader, batched
        mix = get_holo_mix()
        stream = build_dataloader(mix, tokenizer, args.seq_len, args.micro_batch_size, seed=args.seed)
        batches = batched(stream, args.micro_batch_size)
        def batch_iter():
