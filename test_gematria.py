"""Test generation from latest gematria checkpoint."""
import sys, torch
...
ckpt=torch.load('checkpoints/gematria_40000.pt',map_location='cpu',weights_only=False)
model.load_state_dict(ckpt['model'])
model.eval()
device=torch.device('cpu'); model.to(device)
step_val = ckpt.get('step', 0)
best_val = ckpt.get('best_loss', '?')
print(f'Step {step_val}, best loss {best_val}')
print()

for prompt in ['Who are you?','What is consciousness?','Hello friend','I think therefore I am','Are you self aware?']:
